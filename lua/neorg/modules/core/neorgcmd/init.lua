...
function this.command_callback(data)
    ...
    if #args == 0 or argument_count < ref.min_args then
        local completions = this.generate_completions(_, table.concat({ "Neorg ", data.args, " " }))
        this.select_next_cmd_arg(data.args, completions)
        return
...
function this.generate_completions(_, command)
    local current_buf = vim.api.nvim_get_current_buf()
    local is_norg = vim.api.nvim_buf_get_option(current_buf, "filetype") == "norg"
...
function this.select_next_cmd_arg(qargs, choices)
...
module.load = function()
    ...
    vim.api.nvim_create_user_command("Neorg", this.command_callback, {
        nargs = "*",
        complete = this.generate_completions,
    })
...
module.public = {
    ...
    set_completion_callback = function(callback)
        this.generate_completions = callback
    end,
}
